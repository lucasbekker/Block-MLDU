...
function [ L_MXs, D_MXs, U_MXs, S_nz, A_nz ] = MLDU_test6( A, block_sizes )
...
A_nz = zeros(length(block_sizes),1);
...
for k = 1:length(block_sizes)
    ...
    % Speed optimization, compress the dynamically growing matrix if the
    % condition stated in the 'compress_sanity_check' is met.
    compress = compress_sanity_check_local(l_compress,S_nz(k));
...
function [ ] = Block_Join_S_nested( ind_S, block_size, compress )
    ...
    if compress == 1
        [S.i,S.j,S.v] = find(sparse([S.i(ind_S) - block_size; i],...
                             [S.j(ind_S) - block_size; j],[S.v(ind_S) ; v]));
    else
...
function [ compress ] = compress_sanity_check_local( l, nz )
